...
The available built-in query strategies are *max uncertainty sampling*, *max margin sampling* and *entropy sampling*. For more details, see the `Uncertainty sampling <../query_strategies/Uncertainty-sampling.html>`__ page.

Query strategies
----------------

In modAL, currently there are three built-in query strategies: *max uncertainty*, *max margin* and *max entropy*, they are located in the ``modAL.uncertainty`` module. You can find an `informal tutorial here <../query_strategies/Uncertainty-sampling.html>`__ about them.
